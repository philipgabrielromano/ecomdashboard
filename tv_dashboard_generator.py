#!/usr/bin/env python3
"""Generate a 4K 16:9 TV dashboard HTML + PNG from Upright Labs / Lister sales data."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a 4K 16:9 TV dashboard from Upright Labs sold-item data."
    )
    ...
    parser.add_argument("--png-output", help=f"Output PNG path. Defaults to {DEFAULT_PNG_OUTPUT}")
    parser.add_argument("--json-output", help=f"Output metadata JSON path. Defaults to {DEFAULT_JSON_OUTPUT}")
    parser.add_argument("--title", help="Dashboard title.")
    parser.add_argument("--organization-name", help="Optional organization/display name shown in footer.")
    ...
    parser.add_argument("--refresh-seconds", type=int, default=300, help="Auto-refresh interval.")
...
def build_session(token: str) -> requests.Session:
    ...
    session.headers.update({
        ...
        "User-Agent": "uprightlabs-tv-dashboard-4k/1.0",
    })
    return session
...
def fmt_generated_at(dt: datetime) -> str:
...
GENERIC_BRAND_TERMS = {
    "new", "lot", "set", "vintage", "pair", "size", "mens", "men", "womens", "women",
    "kids", "bulk", "mixed", "rare", "authentic", "style", "large", "small", "used",
    "tested", "working", "with", "tag", "tags", "nwt", "nwot"
}


def clean_brand_token(token: str) -> str:
...
def infer_brand(title: str) -> str:
    ...
    if len(usable) >= 2 and usable[1] == "&" and len(usable) >= 3:
        return f"{usable[0]} & {usable[2]}"

    if len(usable) >= 2 and usable[0].lower() in {"the"}:
        return usable[1]
...
def normalize_collectible_brand(brand: str, title: str) -> str:
    ...
    if brand_lower in generic_terms:
        return ""

    if brand_lower in GENERIC_BRAND_TERMS:
        return ""
...
def build_kpis(summary: Dict[str, Any]) -> str:
    ...
    for idx, item in enumerate(featured, start=1):
        ...
        out.append(
            ...
            <article class="featured-card">
              <div class="featured-rank">#{idx}</div>
              <div class="featured-media">{img}</div>
              <div class="featured-supplier-ribbon">{esc((item.get("supplier") or "Unknown Supplier").upper())}</div>
              <div class="featured-body">
                <div class="featured-title">{esc(truncate_text(item["title"]))}</div>
                <div class="featured-price">{fmt_currency(item["item_price"])}</div>
                <div class="featured-details-row">{esc(details_line)}</div>
              </div>
            </article>
            '''
        )
...
def render_tv_html(args: argparse.Namespace, summary: Dict[str, Any], featured: List[Dict[str, Any]], generated_at: datetime) -> str:
    report_span = fmt_report_span(args.start, args.end)
    generated_label = fmt_generated_at(generated_at)
    logo_uri = logo_data_uri(args.logo_path)
    logo_html = f'<div class="logo-shell"><img src="{logo_uri}" alt="Logo" /></div>' if logo_uri else ''
    org = esc(args.organization_name) if args.organization_name else ''
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .screen {{
      ...
      padding: 56px;
      background:
        radial-gradient(circle at top right, rgba(178,210,53,0.22), transparent 22%),
        linear-gradient(135deg, #00539F 0%, #004885 60%, #003867 100%);
      display: grid;
      grid-template-rows: 252px 248px 1fr 72px;
      gap: 30px;
    }}
    ...
    .header {{
      ...
      padding: 34px 40px;
      display: grid;
      grid-template-columns: 260px 1fr 740px;
      gap: 28px;
      align-items: center;
    }}
    ...
    .report-box {{
      ...
      border-left: 10px solid var(--goodwill-light);
      border-radius: 28px;
      padding: 22px 26px;
    }}
    ...
    .kpi-card {{
      ...
      border-top: 12px solid var(--goodwill-light);
      border-radius: 30px;
      box-shadow: var(--shadow);
    ...
    .panel-header {{
      ...
      border-bottom: 3px solid rgba(178,210,53,0.55);
      color: var(--goodwill-blue);
      font-size: 46px;
      font-weight: 900;
    ...
    .featured-card {{
      background: white;
      border: 3px solid rgba(0,83,159,0.10);
      border-top: 12px solid var(--goodwill-light);
      border-radius: 30px;
      overflow: hidden;
      display: grid;
      grid-template-rows: 74px 430px 64px 1fr;
      min-height: 0;
    }}
    ...
    .featured-media {{
      background: #f6fbdc;
      border-bottom: 2px solid rgba(0,83,159,0.10);
      display: flex;
      align-items: center;
      justify-content: center;
      padding: 22px;
  ...
  <main class="screen">
    <section class="header">
      ...
      <div>
        ...
        <div class="header-subline">Top suppliers, top brands, and highest-value items from the last 3 days</div>
      </div>
    ...
    <footer class="footer">
      <div class="left">{org if org else "Generated by tv_dashboard_generator_4k.py using Upright Labs data."}</div>
      <div class="right">Generated: {esc(generated_label)}</div>
...
def write_metadata_json(json_path: str, args: argparse.Namespace, summary: Dict[str, Any], featured: List[Dict[str, Any]], generated_at: datetime) -> None:
    payload = {
        "title": args.title,
...
def main() -> None:
    ...
    if not args.tag:
        env_tags = (os.getenv("TV_DASHBOARD_TAGS") or "").strip()
    ...
    args.organization_name = args.organization_name or os.getenv("TV_DASHBOARD_ORGANIZATION", "")
    args.image_mode = args.image_mode or os.getenv("TV_DASHBOARD_IMAGE_MODE", "remote")

    verbose = True if args.verbose else True
    generated_at = datetime.now(timezone.utc)

    log("Starting 4K TV dashboard generation...", verbose)
    ...
    featured = enrich_items(session, featured_raw, args, product_cache={})
    summary = compute_summary(filtered, featured)
    html_out = render_tv_html(args, summary, featured, generated_at)
    ...
    html_to_png(str(output_path), str(png_path))

    write_metadata_json(str(Path(args.json_output)), args, summary, featured, generated_at)
